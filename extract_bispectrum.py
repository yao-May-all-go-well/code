...
def plot_polycoherence(freq1, freq2, bicoh, figsize=(8, 4)):
    df1 = freq1[1] - freq1[0]
    df2 = freq2[1] - freq2[0]
    freq1 = np.append(freq1, freq1[-1] + df1) - 0.5 * df1
    freq2 = np.append(freq2, freq2[-1] + df2) - 0.5 * df2

    plt.figure(figsize=figsize, dpi=300)  # 创建一个宽8英寸、高4英寸的画布

    # 第一个子图
    ax1 = plt.subplot(1, 2, 2)  # 1行2列的第1个
    plt.pcolormesh(freq2, freq1, np.abs(bicoh), cmap=plt.cm.jet)
    plt.axis('off')

    # 第二个子图
    ax2 = plt.subplot(1, 2, 1)  # 1行2列的第2个
    plt.contour(freq2[:-1], freq1[:-1], np.abs(bicoh), 10, linewidths=1, cmap=plt.cm.jet)
